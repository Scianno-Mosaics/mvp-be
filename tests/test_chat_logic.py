import pytest
from mvp_be.services import api_logic
...
async def test_handle_space_query_success(monkeypatch):
    ...
    response = await api_logic.handle_space_query()
...
def test_handle_time_query_format():
    result = api_logic.handle_time_query()
    assert result.startswith("🕒 The current UTC time is")
    assert "UTC on" in result


def test_handle_joke_request():
    result = api_logic.handle_joke_request()
    assert result.startswith("😂")
    assert any(joke in result for joke in api_logic.jokes)


def test_handle_help_request():
    result = api_logic.handle_help_request()
...
def test_handle_default_echo():
    message = "hello"
    result = api_logic.handle_default_echo(message)
